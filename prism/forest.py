...
线程安全(关键): agent 跑在 actor 线程(原则 15), forest 可能在 TUI 线程建、
  actor 线程用 → check_same_thread=False + threading.Lock 串行化所有 DB 操作。
  约定: 公开方法持锁; 私有 helper(_load_meta 等)由调用方持锁后调用(锁不可重入)。
...
class SQLiteForest(Forest):
    """认知树存储(SQLite + 递归 CTE + WAL, 线程安全)。一个实例绑一个 session_id。"""
    ...
    def __init__(self, db_path, session_id: str = "default"):
        ...
        # check_same_thread=False: 允许跨线程(actor 线程用); Lock 保证串行化安全
        ...
        with self._lock:
            ...
            if parent is not None:
                ...
                self._conn.execute(
                    ...
                    (parent, CAUSES, nid))   # causes: parent → child
            self._conn.commit()
            return nid
        ...
        with self._lock:
            if depth is not None:
                rows = self._conn.execute(
                    ...
                    (node, relation, relation, depth)).fetchall()
            else:
                rows = self._conn.execute(
                    ...
                    ") SELECT n.* FROM nodes n JOIN reach ON n.id=reach.id ORDER BY n.created_at",
                    (node, relation, relation)).fetchall()
            return [dict(r) for r in rows]
    ...
    def recent_in_tree(self, tree_id: int, *, type: str | None = None,
        ...
        SQL 全静态分支 + 参数化(LIMIT ? 传 -1 = 不限), 无动态拼接。
        """
        with self._lock:
            if type is not None and status is not None:
                rows = self._conn.execute(
                    "SELECT * FROM nodes WHERE session_id=? AND tree_id=? "
                    "AND type=? AND status=? ORDER BY id DESC LIMIT ?",
                    (self.session_id, tree_id, type, status,
                     -1 if limit is None else limit)).fetchall()
            elif type is not None:
                rows = self._conn.execute(
                    "SELECT * FROM nodes WHERE session_id=? AND tree_id=? "
                    "AND type=? ORDER BY id DESC LIMIT ?",
                    (self.session_id, tree_id, type,
                     -1 if limit is None else limit)).fetchall()
            elif status is not None:
                rows = self._conn.execute(
                    "SELECT * FROM nodes WHERE session_id=? AND tree_id=? "
                    "AND status=? ORDER BY id DESC LIMIT ?",
                    (self.session_id, tree_id, status,
                     -1 if limit is None else limit)).fetchall()
            else:
                rows = self._conn.execute(
                    "SELECT * FROM nodes WHERE session_id=? AND tree_id=? "
                    "ORDER BY id DESC LIMIT ?",
                    (self.session_id, tree_id,
                     -1 if limit is None else limit)).fetchall()
